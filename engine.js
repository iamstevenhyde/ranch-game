...
const BULLS_PER_YEAR = 6;     // real median sale = 6 lots (kept for compatibility; year 1 runs lighter)
const DISCOUNT = 0.92;

// sale lots by year (Steven 9/02, speed pass): year 1 runs a lighter 4-lot sale so the
// first board meeting is not overloaded before players know the game; year 2+ is the
// full 6-lot median sale. BULLS_PER_YEAR stays as the engine default for compatibility
// with anything that reads the flat constant.
function bullsForYear(year) { return year <= 1 ? 4 : BULLS_PER_YEAR; }
...
const UNLOCKS = { debt: 3, tech: 2, deals: 2 };
...
// ---------- labor as the O gate (Penrose effect, Steven 9/02 spec) ----------
// Owning a bull is access (RVIO's V, R, I); MANAGING him (breeding program, records,
// calving oversight) is O. A ranch's `hands` is management capacity, separate from the
// HANDS_PER_COW operating-labor cost above. Each active bull needs HANDS_PER_BULL of
// that capacity; short-handed ranches still own the bull but realize less of his true
// genetics into the herd (Penrose 1959: growth is capped by managerial capacity, and
// new capacity takes a year to become useful, hence the hire lag below).
...
// (family_survival, conservative) and two start cash-rich/hands-poor (elite_genetics,
// seedstock), so the same bull is worth different money partly because of O.
function startingHands(l, key) {
  const mult = (ARCHETYPES[key] && ARCHETYPES[key].handsMult) || 1.0;
  // tie starting crew to what a FULL bull roster would need (not raw herd size, which
  // would over-hire big-herd archetypes far past their roster cap and crush them on
  // payroll alone); everyone starts partially staffed (half a full roster) so the O gate
  // binds sometimes but is never a starting death sentence.
  const roster = clamp(Math.round(l.herd / 250), 2, 8);
  const need = roster * HANDS_PER_BULL;
  return clamp(+(need * 0.5 * mult).toFixed(2), 0.5, 9);
}
// per-year hire cap and per-hand cost both ride region laborScarcity (Great Basin:
// scarcity 1.9 -> cap 1/yr and the priciest hands; the other regions cap at 2/yr).
function hireCap(r) { return REGIONS[r.region].laborScarcity >= 1.7 ? 1 : 2; }
function hireCost(r, w) { return Math.round(HAND_WAGE * w.wageRatchet * REGIONS[r.region].laborScarcity); }
// management capacity required THIS year, net of tech laborSave (same stat the
...
// lag: hands hired last year become effective now. Call at year-open, before the hire
// decision for the new year is set (decideYear does this for AI; team.html must call it
// for the human board phase, mirroring how it mirrors every other decideYear step).
function activateHands(r) {
  if (r.handsPending) { r.hands += r.handsPending; r.handsPending = 0; }
}
// one board decision per year: hire 0/+1/+2 hands, capped by region scarcity. Effective
// NEXT year (activateHands), never this one. `n` is the raw board choice (hire field).
function setHire(r, n) {
  r.handsPending = clamp(Math.round(n || 0), 0, hireCap(r));
  return r.handsPending;
}
...
const ARCHETYPES = {
  cost_leader: {
    ...
    shade: 0.70, bidCashCap: 0.25, expandRate: 0.15, debtCap: 0.25, leanOps: 0.95, handsMult: 1.3, // scale outfit, established crew
  ...
  conservative: {
    ...
    shade: 0.70, bidCashCap: 0.15, expandRate: 0.05, debtCap: 0.0, handsMult: 1.4,
...
function productionYear(r, w) {
  ...
  // O-gate payroll: the management crew's annual wage, independent of the herd-scaled
  // operating labor above. Charged on hands actually on the payroll THIS year, whether
  // or not they are fully utilized (idle capacity still draws a paycheck).
  const payroll = r.hands * hireCost(r, w);
  // fixed overhead (equipment, insurance, facilities) is lumpy: small outfits pay it too
  const costs = (cowCostTot + laborTot) * lean + overhead + extraCost + commodityBullCost + interest + payroll;
  ...
  r.lastStmt = { year: w.year, revenue, semenRoyalty, semenStraws, tallowNet: tal.net, tallowCulls: Math.round(tal.culls), cullCash: Math.round(cullCash), cowCost: cowCostTot * lean, labor: laborTot * lean,
                 overhead, bullCost: commodityBullCost, breeding: extraCost, interest, payroll, net,
  ...
  // O gate: owning a bull is not exploiting him. A short-handed crew (utilization<1)
  // realizes traitVal * utilization^K of his true genetics into the herd; the bull is
  // owned but under-managed, exactly the Penrose-effect thesis the labor mechanic exists
  ...
  // scorecard lines: resources OWNED (active bulls) vs resources EXPLOITED (utilization),
  // plus a dollar estimate of what the understaffing cost this year (display only; the
  // real cost already landed above as a smaller genetics pull, which compounds forward)
  let utilPenalty = 0;
  if (active.length && util < 1) {
...
function decideYear(r, w) {
  const a = r.a;
  // O gate: last year's hire decision becomes effective capacity now (Penrose lag).
  // Must run before anything below reads r.hands / utilization.
  activateHands(r);
  ...
  // (the lag punishes reacting late). Estimate next year's active roster as this year's
  // plus one more slot if this archetype still bids at auction and has roster room; cash-
  // poor archetypes hire less often purely because they fail the affordability check, so
  // the policy stays archetype-consistent without any special-casing here. Not year-gated:
  // unlike tech, hiring is a labor decision available from year one.
  {
...
function auctionYear(ranches, w, ledger) {
  ...
  for (let i = 0; i < bullsForYear(w.year); i++) {
